...
router.post("/", function (req, res, next) {
  ...
  // 和資料庫的密碼比對
  db.get("SELECT Password FROM `Accounts` WHERE StuID = ?", [user], (error, passwd)=>{
...
router.get("/temperature", function (req, res, next){
  ...
  db.get("SELECT `Name` FROM `Accounts` WHERE `StuID` = ?",[user],(error, rows)=>{
...
router.get("/tempok", function (req, res, next){
  let user = req.session['user'];
  let tempok = "37.5";
  db.get("SELECT `Name` FROM `Accounts` WHERE `StuID` = ?",[user],(error,name)=>{
